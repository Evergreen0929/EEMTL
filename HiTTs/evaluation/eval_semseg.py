...
# CITYSCAPES_CATEGORY_NAMES = ['unlabelled', 'road', 'sidewalk', 'building', 'wall', 'fence',\
...
CITYSCAPES_CATEGORY_NAMES = [str(i) for i in range(7)]
...
class SemsegMeter(object):
    ...
    def update(self, pred, gt):
        ...
        if self.ignore_idx == None:
            valid = (gt >= 0) & (gt < self.n_classes) # -1 as ignore index, for nyud_low
        else:
...
def eval_semseg_predictions(p, database, save_dir, overfit=False):
