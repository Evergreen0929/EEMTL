...
class Spatial_CrossEntropyLoss(nn.Module):
    """
    Cross entropy loss with ignore regions.
    """
    def __init__(self, ignore_index=255, class_weight=None, balanced=False, threshold=0.6):
        super().__init__()
        self.ignore_index = ignore_index
        if balanced:
            assert class_weight is None
    ...
    def forward(self, out, label, score, reduction='mean'):
        label = torch.squeeze(label, dim=1).long()

        score = (score > self.threshold).to(dtype=torch.float32)

        if self.balanced:
            mask = (label != self.ignore_index)
            masked_label = torch.masked_select(label, mask)
            # masked_score = torch.masked_select(score, mask)
            # assert torch.max(masked_label) < 2  # binary
            num_labels_neg = torch.sum(1.0 - masked_label)
            num_total = torch.numel(masked_label)
            w_pos = num_labels_neg / num_total
...
class Spatial_L1Loss(nn.Module):
    """
    L1 loss with ignore regions.
    normalize: normalization for surface normals
    """
    def __init__(self, normalize=False, ignore_index=255, threshold=0.6):
        super().__init__()
        self.normalize = normalize
        self.ignore_index = ignore_index
        self.threshold = threshold

    def forward(self, out, label, score):

        if self.normalize:
            out = nn.functional.normalize(out, p=2, dim=1)

        score_mask = (score > self.threshold).to(dtype=torch.float32)
        score = score * score_mask

        mask = (label != self.ignore_index).all(dim=1, keepdim=True).to(dtype=torch.float32)
        loss = nn.functional.l1_loss(out, label, reduction='none') * mask * score

        return loss.mean()

class Spatial_BalancedBinaryCrossEntropyLoss(nn.Module):
    """
    Balanced binary cross entropy loss with ignore regions.
    """
    def __init__(self, pos_weight=None, ignore_index=255, threshold=0.6):
        super().__init__()
        self.pos_weight = pos_weight
        self.ignore_index = ignore_index
        self.threshold = threshold

    def forward(self, output, label, score, reduction='mean'):

        mask = (label != self.ignore_index) * (score < self.threshold)
